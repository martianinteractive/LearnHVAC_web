class Admins::ClientVersionsController < Admins::ApplicationController
  ...
  def create
    ...
    if @client_version.save
      redirect_to(admins_client_version_path(@client_version), :notice => 'Version was successfully created.')
    else
      render :action => "new"
    end
  end
  ...
  def update
    ...
    if @client_version.update_attributes(params[:client_version])
      redirect_to(admins_client_version_path(@client_version), :notice => 'Version was successfully updated.')
    else
      render :action => "edit"
    end
  end
  ...
  def destroy
    ...
    redirect_to(admins_client_versions_path)
  end
