class Instructor::VariablesController < Instructor::ApplicationController
  ...
  def create
    ...
    if @scenario_variable.save
      redirect_to(instructor_scenario_variable_path(@scenario, @scenario_variable), :notice => 'ScenarioVariable was successfully created.')
    else
      render :action => "new"
    end
  end
  ...
  def update
    if @scenario_variable.update_attributes(params[:scenario_variable])
      redirect_to(instructor_scenario_variable_path(@scenario, @scenario_variable), :notice => 'ScenarioVariable was successfully updated.')
    else
      render :action => "edit"
    end
  end
  ...
  def destroy
    ...
    redirect_to(instructor_scenario_variables_path(@scenario))
  end
